...
`use-manual-expense-dialog-form.ts`: al abrir el diálogo controlado desde el padre (`isOpen`), se reinicia el formulario en render al detectar transición `isOpen` / `profileId` con un snapshot en `useState` (`dialogSnapshot`), patrón recomendado en React para reset por props — no con `useEffect` ni mutando refs en render.
