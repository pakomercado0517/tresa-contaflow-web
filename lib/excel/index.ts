...
import { validateExcelOptions, validateExcelProfilesOptions } from './utils/validators';
...
export async function exportToExcel(options: ExcelOptions): Promise<void> {
  ...
  if (tipo === 'facturas') {
    const periodo = mes >= 1 && año >= 1 ? `Período: ${MESES[mes - 1]} ${año}` : 'Reporte general';
...
export type {
  ExcelOptions,
  ExcelInvoicesOptions,
  ExcelExpensesOptions,
  ExcelProfilesOptions,
  ExcelMetrics,
  ProfileStats,
} from './core/types';
