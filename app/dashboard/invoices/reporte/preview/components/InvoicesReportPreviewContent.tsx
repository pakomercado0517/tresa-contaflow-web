...
export function InvoicesReportPreviewContent({ data }: InvoicesReportPreviewContentProps) {
  ...
  const reportFileName = `contafy-facturas-${MESES[data.mes - 1].toLowerCase()}-${data.año}.pdf`;
  const reportShareTitle = `Reporte de facturas Contafy - ${data.periodoLabel}`;
  const reportShareText = `Reporte detallado de facturas - ${data.profileName}`;
