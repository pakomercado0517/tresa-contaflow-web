...
import type { FilaGastoReporte } from "./reporte-gastos-types";

interface ReporteGastosTableProps {
  filas: FilaGastoReporte[];
}

export function ReporteGastosTable({ filas }: ReporteGastosTableProps) {
            ...
            filas.map((fila, idx) => (
              ...
              >
                <td className="px-4 py-2.5 text-gray-900">
                  {formatReporteGastosDate(fila.fecha)}
                </td>
                ...
                <td className="px-4 py-2.5 font-mono text-xs text-gray-700">{fila.rfcEmisor}</td>
                <td className="px-4 py-2.5 text-right text-gray-900">
                  {formatReporteGastosCurrency(fila.subtotal)}
                </td>
                <td className="px-4 py-2.5 text-right text-gray-900">
                  {formatReporteGastosCurrency(fila.impuestos)}
                </td>
                <td className="px-4 py-2.5 text-right font-medium text-amber-700">
                  {formatReporteGastosCurrency(fila.total)}
                </td>
              </tr>
            ))
          )}
          ...
          <tfoot>
            <tr className="border-t-2 border-amber-200 bg-amber-50/40 font-semibold">
              ...
              <td className="px-4 py-3 text-right text-gray-900">
                {formatReporteGastosCurrency(subtotalReporte)}
              </td>
              <td className="px-4 py-3 text-right text-gray-900">
                {formatReporteGastosCurrency(impuestosReporte)}
              </td>
              <td className="px-4 py-3 text-right text-amber-700">
                {formatReporteGastosCurrency(totalReporte)}
              </td>
            </tr>
          </tfoot>
        )}
